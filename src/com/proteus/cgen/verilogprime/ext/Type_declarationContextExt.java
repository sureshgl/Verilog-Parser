...
import com.proteus.cgen.verilogprime.gen.VerilogPrimeParser.Type_declarationContext;

public class Type_declarationContextExt extends AbstractBaseExt{

	@Getter private Type_declarationContext ctx;
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof Type_declarationContext){
				this.ctx = (Type_declarationContext) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+Type_declarationContext.class.getName());
			}
		} else {
			this.ctx= null;
		}
