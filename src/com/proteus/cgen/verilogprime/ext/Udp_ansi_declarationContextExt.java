...
import com.proteus.cgen.verilogprime.gen.VerilogPrimeParser.Udp_ansi_declarationContext;

public class Udp_ansi_declarationContextExt extends AbstractBaseExt{

	@Getter private Udp_ansi_declarationContext ctx;
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof Udp_ansi_declarationContext){
				this.ctx = (Udp_ansi_declarationContext) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+Udp_ansi_declarationContext.class.getName());
			}
		} else {
			this.ctx= null;
		}
