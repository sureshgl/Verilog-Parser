...
public class List_of_port_identifiers_part1ContextExt extends AbstractBaseExt{

	@Getter private List_of_port_identifiers_part1Context ctx;

	public List_of_port_identifiers_part1ContextExt(List_of_port_identifiers_part1Context ctx) {
		this.ctx = ctx;
	}
	...
	public ParserRuleContext getContext(String str){
		return new PopulateExtendedContextVisitor().visit(getPrimeParser(str).list_of_port_identifiers_part1());
	}

	@Override
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof List_of_port_identifiers_part1Context){
				this.ctx = (List_of_port_identifiers_part1Context) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+List_of_port_identifiers_part1Context.class.getName());
			}
		} else {
			this.ctx= null;
		}
