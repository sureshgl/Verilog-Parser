...
import com.proteus.cgen.verilogprime.gen.VerilogPrimeParser.Setup_timing_checkContext;

public class Setup_timing_checkContextExt extends AbstractBaseExt{

	@Getter private Setup_timing_checkContext ctx;
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof Setup_timing_checkContext){
				this.ctx = (Setup_timing_checkContext) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+Setup_timing_checkContext.class.getName());
			}
		} else {
			this.ctx= null;
		}
