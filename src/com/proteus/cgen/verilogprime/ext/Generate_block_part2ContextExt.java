...
import com.proteus.cgen.verilogprime.gen.VerilogPrimeParser.Generate_block_part2Context;

public class Generate_block_part2ContextExt extends AbstractBaseExt{

	@Getter private Generate_block_part2Context ctx;
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof Generate_block_part2Context){
				this.ctx = (Generate_block_part2Context) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+Generate_block_part2Context.class.getName());
			}
		} else {
			this.ctx= null;
		}
