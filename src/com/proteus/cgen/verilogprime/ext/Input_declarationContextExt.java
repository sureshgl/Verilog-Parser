...
import com.proteus.cgen.verilogprime.gen.VerilogPrimeParser.Input_declarationContext;
...
public class Input_declarationContextExt extends AbstractBaseExt{
	...
	@Getter private Input_declarationContext ctx;
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof Input_declarationContext){
				this.ctx = (Input_declarationContext) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+Input_declarationContext.class.getName());
			}
		} else {
			this.ctx= null;
		}
