...
import com.proteus.cgen.verilogprime.gen.VerilogPrimeParser.Fullskew_timing_checkContext;

public class Fullskew_timing_checkContextExt extends AbstractBaseExt{

	@Getter private Fullskew_timing_checkContext ctx;
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof Fullskew_timing_checkContext){
				this.ctx = (Fullskew_timing_checkContext) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+Fullskew_timing_checkContext.class.getName());
			}
		} else {
			this.ctx= null;
		}
