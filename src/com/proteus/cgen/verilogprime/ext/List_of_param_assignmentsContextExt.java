...
public class List_of_param_assignmentsContextExt extends AbstractBaseExt{
	...
	@Getter private List_of_param_assignmentsContext ctx;
	...
	public void setContext(ParserRuleContext ctx){
		if(ctx != null){
			if(ctx instanceof List_of_param_assignmentsContext){
				this.ctx = (List_of_param_assignmentsContext) ctx;
			} else {
				throw new ClassCastException(ctx.getClass().getSimpleName() + " cannot be cased to "+List_of_param_assignmentsContext.class.getName());
			}
		} else {
			this.ctx= null;
		}
